...
/* 发布者函数 */
extern "C" int publisher_main(int domainId, int sample_count,string str)
{
	...
	memcpy_s(instance->m_data,str.size(), str.c_str(),str.size());
	MD5 md5(string(instance->m_data));

	//string str = md5.toString();
	memset(instance->m_md5, 0, MD5LENG + 1);
	memcpy_s(instance->m_md5, MD5LENG, md5.toString().c_str(), MD5LENG);
	/* 7. 主循环 ，发送数据 */
	double t = 0;
	double nMaxt = 0;
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		nMaxt += (double)(end - begin) / CLOCKS_PER_SEC;
		
		if (t >= 1.0)
		{
			cout << "吞吐量为:" << (count + 1)*str.size() * 8.0 << "Mbps" << endl;
			count = -1;
			t = 0;
		}
		if (nMaxt > 10.0)
...
int main(int argc, char *argv[])
{
	...
	cout << "---------------功能测试--------------" << endl;
	publisher_main(domain_id, 1,"hello world");

	string str;
	cout<<"---------------------------------------"<<endl;
	cout << "吞吐量测试（测试时间10s）请输入测试数据:";
	cin >> str;
	publisher_main(domain_id, sample_count,str);
}
